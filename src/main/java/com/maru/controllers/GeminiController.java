...
public class GeminiController {
    ...
    @Operation(
        summary = "Gemini 대화 컨텍스트 유지 텍스트 생성",
        description = "Gemini API를 통해 대화 컨텍스트를 유지하며 한국어로 텍스트를 생성합니다.",
        responses = {
            @ApiResponse(
                responseCode = "200",
                description = "요청 성공",
    ...
    @PostMapping("/chat")
    public GeminiChatResponse generateChatContent(@RequestBody GeminiChatRequest request) {
        Map.Entry<String, List<GeminiMessage>> result = geminiService.generateChatContent(request.getMessages());
        return new GeminiChatResponse(result.getKey(), result.getValue());
    }
    
    @Operation(
        summary = "PDF 분석 및 대화 컨텍스트 유지 응답 생성",
        description = "PDF 파일을 분석하고 Gemini API를 통해 대화 컨텍스트를 유지하며 한국어로 텍스트를 생성합니다.",
        responses = {
            @ApiResponse(
                responseCode = "200",
                description = "요청 성공",
    ...
    @PostMapping(value = "/chat-with-pdf", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public GeminiChatResponse generateChatContentWithPdf(
            @RequestParam(value = "messages", required = false, defaultValue = "[]") String messagesJson,
            @RequestParam("pdfFile") MultipartFile pdfFile) {
        
        // JSON 문자열을 메시지 목록으로 파싱
        List<GeminiMessage> messages = parseMessagesJson(messagesJson);
    ...
    public GeminiChatResponse generateChatContentWithPdfJson(@RequestBody GeminiChatPdfRequest request) {
        ...
        System.out.println("Messages: " + (request.getMessages() != null ? request.getMessages().size() : "null"));
        System.out.println("PdfFileData: " + (request.getPdfFileData() != null ? "not null" : "null"));
    ...
    /**
     * JSON 문자열을 메시지 목록으로 파싱하는 메서드
     */
    private List<GeminiMessage> parseMessagesJson(String messagesJson) {
        List<GeminiMessage> messages = new ArrayList<>();
        
        if (messagesJson != null && !messagesJson.isEmpty() && !messagesJson.equals("[]")) {
            try {
                messages = objectMapper.readValue(messagesJson, new TypeReference<List<GeminiMessage>>() {});
